...
if
 ...
 then
    ...
    local ValidUnitID =
        Spring.ValidUnitID
    local SetUnitNeutral =
        Spring.SetUnitNeutral
    local GetUnitNeutral =
    ...
    local SetToList =
        Spring.Utilities.SetToList
    local SetCount =
        Spring.Utilities.SetCount
    local Round =
        Spring.Utilities.Round
    -- enable when needed
    ...
    do -- load config file
        local CONFIG_FILE
        CONFIG_FILE =
            "LuaRules/Configs/spawn_defs_robots.lua"
        Echo(
            CONFIG_FILE
        )
        local VFSMODE =
    ...
    local function chooseTeamToAttack()
        local numChickens =
            0
        for _, robotTeam in pairs(
            computerTeams
        ) do
            numChickens =
                numChickens +
                robotTeam:GetChickenCount(

                )
        end
        local perTeamEIncome =
            getPerTeamEIncome(
    ...
    local function chooseTarget(
        ...
        return team:addAttackingRobotAndGetTarget(
            unitID
        )
    end
    ...
    function Wave:GetWave(
        ...
        if
            waveSettings.air_fighter.maxcost >
                0
         then
            havePercent =
                havePercent +
                self._airFighterPercent
        end
        ...
        local maxGroundUnits =
            math.ceil(
            maxUnits *
                (self._groundPercent /
                    100)
        )
        local maxAirFighters =
            math.ceil(
            maxUnits *
                (self._airFighterPercent /
                    100)
    ...
    function HumanTeam:removeAttackingRobot(
        unitID)
        if
            not self._attackingRobots[
                unitID
            ]
         then
            return false
        ...
        self._attackingRobots[
                unitID
            ] =
            nil
        self._attackingRobotsCount =
            self._attackingRobotsCount -
            1
        return true
    end

    function HumanTeam:attackThisTeam(
        maxTeamRobots)
        if
            self._attackingRobotsCount >=
                maxTeamRobots
    ...
    --
    function HumanTeam:addAttackingRobotAndGetTarget(
        robotUnitID)
        self._attackingRobotsCount =
            self._attackingRobotsCount +
            1
    ...
    function RobotTeam:SetUp(
        ...
        -- Settings
        self._humanTeamCount =
            SetCount(
            humanTeams
        )
        self._minRobots =
            self.minRobotsPPlayer
        self._maxRobots =
            self.maxRobotsPPlayer
                ...
                if
                 ...
                 then
                    burrowSpawnType =
                        "avoid"
                    Echo(
                        "No Chicken start box available, Burrow Placement set to 'Avoid Players'"
                    )
                elseif
                    (self.lsx1 ==
                        0) and
    ...
    function RobotTeam:_getChickenSpawnLoc(
        burrowID,
        size)
        local x,
            y,
    ...
    function RobotTeam:_spawnBurrows(
        count)
            ...
            repeat
                ...
                if
                 ...
                 then
                    ...
                    local humanUnitsInVicinity =
                        false
                    local humanUnitsInProximity =
                        false
                    ...
                    if
                        (humanUnitsInProximity or
                            not humanUnitsInVicinity)
                     then
                        blocking =
                            1
                    end
    ...
    function RobotTeam:_spawnTurret(
        burrowID,
        turret)
        if
            (mRandom(
                ...
                if
                 ...
                 then
                    ...
                    self._chickenBirths[
                        ...
                        deathDate = self._gameTimeSeconds +
                            self._maxAge,
    ...
    function RobotTeam:_spawnQueen()
        ...
        local sx,
            sy,
            sz
        for burrowID, turretCount in pairs(
            self._burrows
        ) do
    ...
    function RobotTeam:_updateSpawnQueen()
        ...
        chickenEvent(
            "queen"
        ) -- notify unsynced about queen spawn
        _,
            self._queenMaxHP =
            GetUnitHealth(
            self._queenID
    ...
    function RobotTeam:UnitDestroyed(
        unitID,
        unitDefID,
        attackerID)
        if
                 ...
                 then
                    self._queenTime =
                        ...
                        (((Spring.GetModOptions(
                            ...
                            40) *
                            60) *
                            self._survivalQueenMod)
                    self._survivalQueenMod =
                        self._survivalQueenMod *
                        0.8
                    self._queenAnger =
                        0 -- reenable chicken spawning
                    self._burrowAnger =
                        0
                    if
                        self._isBestRobot
                    ...
                    ) -- spawn new chickens (because queen could be the last one)
                else
                    self._gameOver =
                        GetGameFrame(
                    ...
                    -- gameOver handling
                    local globalGameOver =
                        true
                    for _, robotTeam in pairs(
                        computerTeams
                    ) do
                        if
                            not robotTeam:GetGameOver(
        ...
        if
         ...
         then
            ...
            if
                (self._gameTimeSeconds <
                    settingGracePeriod)
             then -- do nothing in the grace period
                return
            end

            -- Update minRobots/maxRobots
            local count =
                GetRealHumanTeamsCount(
            ...
            self._expMod =
                (self._expMod +
                self._expIncrement) -- increment expierence
            ...
            if
                self._queenAnger >=
                    100
             then -- check if the queen should be alive
                self:_updateSpawnQueen(
            ...
            if
                (self.burrowSpawnRate <
                    (self._gameTimeSeconds -
                        self._timeOfLastFakeSpawn) and
                    self._burrowTarget <
                        settingMaxBurrows)
             then
    ...
    function RobotTeam:_updateQueenLife()
        if
            not self._queenID or
                not self._isBestRobot
         then
            return
        end
        ...
        if
            self._queenLifePercent ~=
                lifeCheck
         then -- health changed since last update, update it
            self._queenLifePercent =
                lifeCheck
            SetGameRulesParam(
                "queenLife",
         ...
         then
            if
                (settingModes[
                    ...
                    highestLevel)
             then -- get chicken ai with highest level
                luaAI =
                    teamLuaAI
                highestLevel =
                    settingModes[
    ...
    function gadget:GameStart()
        if
         ...
         then
            Echo(
                "Warning: No Chicken team available, add a Chicken bot"
            )
            Echo(
                "(Assigning Chicken Team to Gaia - AI: Custom)"
            )
        end
            ...
            for teamID in pairs(
                ...
                for _, unitID in ipairs(
                    ...
                    Spring.MoveCtrl.SetPosition(
                        ...
                        Game.mapSizeZ +
                            500
                    ) --don't move too far out or prevent_aicraft_hax will explode it!
    ...
    function gadget:UnitDestroyed(
         ...
         then
            ...
            if
             ...
             then
                ...
                Echo(
                    "BOGUS not Robot Team and not Human Team"
                )
                return
            end
        ...
        if
         ...
         then
            Echo(
                "BOGUS: Not a human team and not a robot team"
            )
            return
        end
    ...
    local Script =
        Script
    local hasChickenEvent =
        false

    local function HasChickenEvent(
        ce)
        hasChickenEvent =
            (ec ~=
            "0")
    ...
    function gadget:Initialize()
        ...
        gadgetHandler:AddChatAction(
            "HasChickenEvent",
            HasChickenEvent,
            "toggles hasChickenEvent setting"
        )
    end

    function gadget:Shutdown()
        gadgetHandler:RemoveChatAction(
            "HasChickenEvent"
        )
    end
end
